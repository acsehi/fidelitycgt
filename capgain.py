...
with open('View open lots.csv') as open_csv:
    ...
    for row in reader:
        if (len(row) > 2 and row[1] != "Quantity"):
            d = datetime.datetime.strptime(row[0], '%b-%d-%Y').strftime('%d/%m/%Y')            
            exchange_rate = exchangetoGBP(datetime.datetime.strptime(row[0], '%b-%d-%Y'))
            date_open.append(d)
...
with open('View closed lots.csv') as open_csv:
    ...
    for row in reader:
        if (len(row) > 2 and row[1] != "Quantity"):
            ...
            exchange_rate_buy = exchangetoGBP(datetime.datetime.strptime(row[0], '%b/%d/%Y'))

            date_open.append(do)
            quantity_open.append(row[1])
            value_open.append((float(row[3])/float(row[1]))*float(exchange_rate_buy))

            # sell
            dc = datetime.datetime.strptime(row[2], '%b/%d/%Y').strftime('%d/%m/%Y')        
            exchange_rate_sell = exchangetoGBP(datetime.datetime.strptime(row[0], '%b/%d/%Y'))
